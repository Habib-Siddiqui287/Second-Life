...
| :--- | :--- | :--- | :--- |
| **Admin** | `admin@secondlife.eco` | Platform Admin | Default admin password: `Admin@12345` |
...
Backend checks:
```bash
python -m compileall -q accounts chatbot config donations services dashboard connections item_requests notifications organizations feedback
...
The repository does not include a guaranteed pre-installed dependency environment. Run these commands on your local machine after installing Python and Node dependencies.

### Email configuration
For registration, password reset, and transactional notifications, configure the backend-only `BREVO_API_KEY`. The sender is `givesthingsasecondlife@gmail.com`. Never put the Brevo key in React/Vite environment variables.

### Production media
Local development uses Django's `media/` directory. For persistent media on a stateless production service, configure the optional S3-compatible variables documented in `.env.example`.
