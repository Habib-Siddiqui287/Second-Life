...
export default function CreateDonationWizard() {
  ...
  const handleImageSelect = async (e) => {
    ...
    try {
      ...
      showToast('Could not upload image. Used default item asset.', 'info');
      setForm((prev) => ({
        ...prev,
        image_urls: [...prev.image_urls, '/images/items/image12.png'],
      }));
    } finally {
      ...
      {step === 3 && (
        <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-soft space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1.5">Weight (Approximate)</label>
              <input
                type="text"
                value={form.weight}
