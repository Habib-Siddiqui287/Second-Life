...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    let mounted=true;
    const load=()=>adminService.getDashboardStats().then(res=>{if(mounted)setData(res)}).catch(()=>{}).finally(()=>{if(mounted)setLoading(false)});
    load(); const interval=setInterval(load,5000); return ()=>{mounted=false;clearInterval(interval)};
  }, []);
        ...
        <div className="lg:col-span-8 bg-white rounded-3xl p-6 border border-slate-100 shadow-soft space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-slate-400">Live database trend • refreshed every 5 seconds</p>
            </div>
            <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-xl animate-pulse">LIVE</span>
          </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={charts.donations_over_time || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="donations" stroke="#15803D" strokeWidth={3} fillOpacity={1} fill="url(#colorDonations)" name="New Donations" />
                <Area type="monotone" dataKey="completed" stroke="#0284C7" strokeWidth={2} fillOpacity={0} name="Completed Handovers" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
