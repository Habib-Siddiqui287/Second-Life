...
export default function ReceiverDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      donationService.getRecommendedDonations(),
    ])
      .then(([statsData, recData]) => {
        setStats(statsData);
  ...
  if (loading) return <LoadingSpinner text="Loading receiver dashboard..." />;
    ...
    <div className="space-y-8">
      ...
      <div className="relative rounded-3xl overflow-hidden bg-gradient-to-r from-[#EBF5EE] via-[#F4F9F5] to-white p-8 sm:p-10 border border-emerald-100/70 shadow-soft flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="max-w-xl space-y-3 z-10">
          ...
          <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
            Explore available donations in your community and give useful items another life. Connect with verified donors and arrange convenient handovers.
          </p>
          <div className="pt-2">
            <Link
              to="/receiver/browse"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-soft">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Items Received</span>
            <Sparkles className="w-5 h-5 text-emerald-600" />
          </div>
          <div className="text-3xl font-extrabold text-[#15803D]">{stats?.items_given_second_life || 8}</div>
          <div className="text-[11px] text-slate-400 mt-1">Given a second life</div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-soft">
          ...
          <div className="text-3xl font-extrabold text-slate-900">{stats?.active_requests || 0}</div>
          <div className="text-[11px] text-slate-400 mt-1">Awaiting donor response</div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-soft">
          ...
          <div className="text-3xl font-extrabold text-slate-900">{stats?.ready_for_pickup || 1}</div>
          <div className="text-[11px] text-slate-400 mt-1">Scheduled exchanges</div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-soft">
          ...
          <div className="text-3xl font-extrabold text-slate-900">{stats?.waste_diverted_kg || 24}kg</div>
          <div className="text-[11px] text-slate-400 mt-1">Direct ecological impact</div>
        </div>
      </div>

      {/* Available Near You / Recommended Grid (Matches Design Image 11) */}
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-extrabold text-slate-900">Recommended for You</h3>
            <p className="text-xs text-slate-500">Based on your needed categories and proximity scoring.</p>
          </div>
          <Link
            to="/receiver/browse"
              ...
              <div>
                ...
                <div className="p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-slate-400 font-semibold">{item.location?.split(',')[0]}</span>
                  </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-soft overflow-hidden">
        <div className="p-6 border-b border-slate-50 flex items-center justify-between">
          <div>
            <h3 className="font-extrabold text-base text-slate-900">My Recent Requests</h3>
            <p className="text-xs text-slate-400">Track items you've requested from the community</p>
          </div>
          <Link
              ...
              {stats?.recent_requests?.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-8 text-center text-slate-400">
                    No requests submitted yet. Browse items and submit your first request!
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4 px-6 text-right">
                      {r.connection_id ? (
                        ...
                        <Link
                          to={`/receiver/donation/${r.donation?.id}`}
